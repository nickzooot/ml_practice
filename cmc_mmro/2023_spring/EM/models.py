...
class BaseAligner(ABC):
    ...
    @abstractmethod
    def align(self, sentences: List[TokenizedSentencePair]) -> List[
        List[Tuple[int, int]]]:
        """
...
def norm(matr, axis=0):
    return matr / np.sum(matr, axis, keepdims=True)


class DiceAligner(BaseAligner):
    def __init__(self, num_source_words: int, num_target_words: int,
                 threshold=0.5):
        self.cooc = np.zeros((num_source_words, num_target_words),
                             dtype=np.uint32)
    ...
    def fit(self, parallel_corpus):
        ...
        self.dice_scores = (2 * self.cooc.astype(np.float32) /
                            (self.cooc.sum(0, keepdims=True) + self.cooc.sum(1,
                                                                             keepdims=True)))
    ...
    def align(self, sentences):
        ...
        for sentence in sentences:
            ...
            for (i, source_token), (j, target_token) in product(
                ...
                if self.dice_scores[
                    source_token, target_token] > self.threshold:
...
class WordAligner(BaseAligner):
    def __init__(self, num_source_words, num_target_words, num_iters):
        ...
        self.translation_probs = np.full((num_source_words, num_target_words),
                                         1 / num_target_words, dtype=np.float32)
        self.num_iters = num_iters

    def _e_step(self, parallel_corpus: List[TokenizedSentencePair]) -> List[
        np.array]:
        """
        ...
        for pair in parallel_corpus:
            #s_tokens = pair.source_tokens
            #t_tokens = pair.target_tokens
            posteriors.append(norm(self.translation_probs[
                                       pair.source_tokens[:, np.newaxis],
                                       pair.target_tokens]))

        return posteriors

    def _compute_elbo(self, parallel_corpus: List[TokenizedSentencePair],
                      posteriors: List[np.array]) -> float:
        """
        ...
        for q, pair in zip(posteriors, parallel_corpus):
            elbo += ((q * np.log(np.maximum(self.translation_probs[
                                            pair.source_tokens[:, np.newaxis],
                                            pair.target_tokens], 1e-8))).sum() -
                     (q * np.log(np.maximum(q.shape[0] * q, 1e-8))).sum())

        return elbo
        pass

    def _m_step(self, parallel_corpus: List[TokenizedSentencePair],
                posteriors: List[np.array]):
        """
        ...
        for q, pair in zip(posteriors, parallel_corpus):
            idx1 = pair.source_tokens[:, np.newaxis]
            idx2 = pair.target_tokens
            np.add.at(self.translation_probs,  (idx1, idx2), q)
        self.translation_probs /= np.sum(self.translation_probs, axis=1,
                                         keepdims=True)
    ...
    def align(self, sentences):
        ...
        for posterior in self._e_step(sentences):
            res.append([(s_idx, t_idx) for t_idx, s_idx in enumerate(posterior.argmax(axis=0)+1, 1)])
        return res
